...
function clampMiniOffset(offset, viewport, bar, anchor) {
    const margin = 8;
...
function WaitingPulseRings({ size }) {
    const pulseA = useRef(new Animated.Value(0)).current;
    const pulseB = useRef(new Animated.Value(0)).current;
    useEffect(() => {
        const makeLoop = (value, delayMs) => Animated.loop(Animated.sequence([
    ...
    return (<View pointerEvents="none" style={[styles.pulseLayer, { width: size, height: size }]}>
      <Animated.View style={[styles.pulseRing, ringStyle(pulseA)]}/>
      <Animated.View style={[styles.pulseRing, ringStyle(pulseB)]}/>
    </View>);
}
function UrgentPulseRings({ size }) {
    const pulseA = useRef(new Animated.Value(0)).current;
    const pulseB = useRef(new Animated.Value(0)).current;
    useEffect(() => {
        const makeLoop = (value, delayMs) => Animated.loop(Animated.sequence([
...
function ParticipantTile({ tile, avatarSize, tileWidth, videoHeight, }) {
    ...
    return (<View style={[
      ...
      {showVideo ? (<View style={[
          ...
          {tile.muted ? (<View style={styles.videoMuteBadge}>
              <Ionicons name="mic-off" size={12} color="#FFFFFF"/>
            </View>) : null}
          {tile.urgent ? (<View style={styles.videoUrgentChip} pointerEvents="none">
              <Text style={styles.urgentBubbleText}>срочная заявка</Text>
            </View>) : null}
        </View>) : (<View style={[styles.avatarWrap, { width: ringBox + 28, height: ringBox + 28 }]}>
          ...
          <View style={[
            ...
            <View style={[styles.avatarSlot, { width: avatarOuter, height: avatarOuter }]}>
              <UserAvatar nickname={tile.name} avatarUrl={tile.avatarUrl} size={avatarSize} badges={tile.badges} frameId={tile.frameId}/>
            </View>
            {tile.muted && !tile.waiting ? (<View style={styles.muteBadge}>
                <Ionicons name="mic-off" size={12} color="#FFFFFF"/>
              </View>) : null}
          </View>
          {tile.urgent ? (<View style={styles.urgentBubble} pointerEvents="none">
              <Text style={styles.urgentBubbleText}>срочная заявка</Text>
            </View>) : null}
        </View>)}
      <NameWithBadges name={tile.isLocal ? `${tile.name} (вы)` : tile.name} badges={tile.badges} textStyle={styles.tileName} badgeSize={12} layout="stack" align="center"/>
...
function buildCallPhaseLabel(opts) {
    ...
    if (isGroup) {
        const liveLabel = liveCount <= 1 ? 'Ждём, кто присоединится' : `${liveCount} в эфире`;
        if (waitingCount > 0) {
            return liveCount <= 1
...
export function VoiceCallOverlay({ visible, minimized = false, title, isGroup = false, status, ringing, error, muted, deafened, cameraOn = false, participants, waitingPeers = [], urgentUntilById = {}, conversationId = null, diceSenderNickname = 'Вы', onToggleMute, onToggleDeafen, onToggleCamera, onHangup, onRetry, onMinimize, onExpand, onUrgentRequest, }) {
    ...
    const beginMiniDrag = useCallback((clientX, clientY) => {
        ...
        const move = (nextX, nextY) => {
            ...
            const next = clampMiniOffset({ x: origin.x + dx, y: origin.y + dy }, { width, height }, miniSizeRef.current, anchor);
    ...
    const waitingCount = waitingPeers.filter((peer) => peer.id && !participants.some((p) => p.identity === peer.id)).length;
    ...
    const statusLine = connection.tone === 'ok' && callPhase
    ...
    const diceLayer = conversationId?.trim() ? (<View pointerEvents={diceOpen ? 'box-none' : 'none'} style={diceOpen ? styles.diceHost : styles.diceHostIdle}>
        <VoiceCallDiceLayer conversationId={conversationId.trim()} senderNickname={diceSenderNickname} open={diceOpen} onOpenChange={setDiceOpen}/>
      </View>) : null;
    if (minimized) {
        ...
        const miniBar = (<View style={pinToTop ? styles.miniPinnedBar : styles.miniBar}>
        <Pressable accessibilityRole="button" accessibilityLabel={pinToTop ? 'Развернуть звонок' : 'Развернуть звонок. Потяните, чтобы перенести'} onPress={() => {
        ...
        onPointerDown={(event) => {
          ...
          <View style={styles.miniCopy}>
            ...
            <Text style={[styles.miniMeta, failed && styles.statusError]} numberOfLines={1}>
              {miniStatusLine}
            </Text>
          </View>
        ...
        <View style={styles.miniActions}>
          {canRollDice ? (<Pressable accessibilityRole="button" accessibilityLabel="Бросить кости" onPress={() => setDiceOpen(true)} style={({ pressed }) => [
                    pinToTop ? styles.miniCtrlCompact : styles.miniCtrl,
                    pressed && styles.pressed,
                ]}>
              <Ionicons name="dice-outline" size={pinToTop ? 17 : 18} color="#FFFFFF"/>
            </Pressable>) : null}
          <Pressable accessibilityRole="button" accessibilityLabel={muted ? 'Включить микрофон (Ctrl+Shift+M)' : 'Выключить микрофон (Ctrl+Shift+M)'} onPress={onToggleMute} style={({ pressed }) => [
                pinToTop ? styles.miniCtrlCompact : styles.miniCtrl,
          ...
          {onToggleCamera && !pinToTop ? (<Pressable accessibilityRole="button" accessibilityLabel={cameraOn ? 'Выключить камеру (Ctrl+Shift+V)' : 'Включить камеру (Ctrl+Shift+V)'} onPress={onToggleCamera} style={({ pressed }) => [
                    styles.miniCtrl,
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Завершить звонок" onPress={onHangup} style={({ pressed }) => [
                pinToTop ? styles.miniCtrlCompact : styles.miniCtrl,
                styles.miniCtrlHangup,
                pressed && styles.pressed,
            ]}>
            <Ionicons name="call" size={pinToTop ? 15 : 16} color="#FFFFFF" style={styles.hangupIcon}/>
          </Pressable>
        </View>
      </View>);
        if (pinToTop) {
    ...
    return (<Modal visible transparent animationType="fade" statusBarTranslucent>
      <View style={styles.modalRoot}>
        <View style={[
          ...
          {onMinimize ? (<Pressable accessibilityRole="button" accessibilityLabel="Свернуть звонок" onPress={onMinimize} style={styles.backdropHit}/>) : null}
        <View style={[styles.shell, isDesktop && styles.shellDesktop]}>
          <View style={styles.header}>
            ...
            linking && styles.statusConnecting,
                ...
                {mediaReady ? (<Text style={styles.timerText} accessibilityLabel={`Длительность ${timerLabel}`}>
                    {timerLabel}
                  </Text>) : null}
              </View>
            </View>
            {onMinimize ? (<Pressable accessibilityRole="button" accessibilityLabel="Свернуть звонок" onPress={onMinimize} style={({ pressed }) => [styles.headerAction, pressed && styles.pressed]}>
                <Ionicons name="remove" size={20} color="#F2F3F5"/>
              </Pressable>) : null}
            {isGroup ? (<View style={styles.headerBadge}>
                <Ionicons name="people" size={14} color="#B5BAC1"/>
                <Text style={styles.headerBadgeText}>{tiles.length}</Text>
              </View>) : null}
          </View>

          {linking || failed ? (<View style={[
                styles.connectionBanner,
                failed ? styles.connectionBannerError : styles.connectionBannerConnecting,
            ]}>
              <Ionicons name={failed ? 'cloud-offline-outline' : 'sync-outline'} size={18} color={failed ? '#FFB4B4' : '#FFE6A8'}/>
              <View style={styles.connectionBannerCopy}>
                ...
                {connection.detail ? (<Text style={styles.connectionBannerDetail} numberOfLines={3}>
                    {connection.detail}
                  </Text>) : null}
              </View>
            </View>) : null}

          <ScrollView style={styles.stageScroll} contentContainerStyle={styles.stageContent} showsVerticalScrollIndicator={false}>
            <View style={[
            styles.grid,
            {
              ...
              {tiles.map((tile) => (<ParticipantTile key={tile.key} tile={tile} avatarSize={layout.avatarSize} tileWidth={layout.tileWidth} videoHeight={layout.videoHeight}/>))}
            </View>
          </ScrollView>

          <View style={styles.controls}>
            {failed && onRetry ? (<Pressable accessibilityRole="button" accessibilityLabel="Повторить" onPress={onRetry} style={({ pressed }) => [
                styles.controlBtn,
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={muted ? 'Включить микрофон (Ctrl+Shift+M)' : 'Выключить микрофон (Ctrl+Shift+M)'} disabled={!mediaReady} onPress={onToggleMute} style={({ pressed }) => [
            styles.controlBtn,
            ...
            {onToggleCamera ? (<Pressable accessibilityRole="button" accessibilityLabel={cameraOn ? 'Выключить камеру (Ctrl+Shift+V)' : 'Включить камеру (Ctrl+Shift+V)'} disabled={!mediaReady} onPress={onToggleCamera} style={({ pressed }) => [
                styles.controlBtn,
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={deafened ? 'Включить звук (Ctrl+Shift+D)' : 'Отключить звук (Ctrl+Shift+D)'} disabled={!mediaReady} onPress={onToggleDeafen} style={({ pressed }) => [
            styles.controlBtn,
            deafened ? styles.controlBtnMuted : styles.controlBtnSecondary,
            pressed && styles.pressed,
            !mediaReady && styles.controlDisabled,
        ]}>
              <MaterialCommunityIcons name={deafened ? 'headphones-off' : 'headphones'} size={24} color="#FFFFFF"/>
            </Pressable>
            ...
            {onUrgentRequest ? (<Pressable accessibilityRole="button" accessibilityLabel="Срочная заявка" disabled={!mediaReady} onPress={onUrgentRequest} style={({ pressed }) => [
                styles.controlBtn,
