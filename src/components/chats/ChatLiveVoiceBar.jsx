...
function statusLabel(status, participants, error, ringing) {
    if (status === 'error') {
...
export function ChatLiveVoiceBar({ status, ringing, error, muted, participants, onToggleMute, onLeave, onRetry, }) {
    ...
    return (<View style={[styles.bar, barTint]}>
      <View style={styles.left}>
        {linking ? (<ActivityIndicator size="small" color="#C98900"/>) : (<View style={[
                styles.dot,
                {
      ...
      <View style={styles.actions}>
        {failed && onRetry ? (<Pressable accessibilityRole="button" accessibilityLabel="Повторить" hitSlop={8} onPress={onRetry} style={({ pressed }) => [styles.iconBtn, pressed && styles.pressed]}>
            <Ionicons name="refresh" size={18} color={colors.primary}/>
          </Pressable>) : null}

        {mediaReady ? (<Pressable accessibilityRole="button" accessibilityLabel={muted ? 'Включить микрофон' : 'Выключить микрофон'} hitSlop={8} onPress={onToggleMute} style={({ pressed }) => [
                styles.iconBtn,
                muted && styles.muteOn,
                pressed && styles.pressed,
            ]}>
            <Ionicons name={muted ? 'mic-off' : 'mic'} size={18} color={muted ? colors.destructive : colors.primary}/>
          </Pressable>) : null}

        <Pressable accessibilityRole="button" accessibilityLabel="Выйти из голосового" hitSlop={8} onPress={onLeave} style={({ pressed }) => [styles.hangup, pressed && styles.pressed]}>
