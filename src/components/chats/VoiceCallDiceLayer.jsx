...
export function VoiceCallDiceLayer({ conversationId, senderNickname, open, onOpenChange, }) {
    ...
    const handleRoll = useCallback(async (input) => {
    ...
    return (<>
      <ChatDicePopover visible={open} busy={busy} onClose={() => onOpenChange(false)} onRoll={(input) => void handleRoll(input)}/>
      <ChatDiceOverlay request={localRoll ? null : incoming} localRoll={localRoll} onLocalRollComplete={handleLocalComplete} warm={open || busy || Boolean(localRoll) || Boolean(incoming)} forceActive onReveal={() => undefined} onAdvance={handleIncomingAdvance}/>
    </>);
}
