...
import { isDiceSkinId, skinAccent } from '@/data/rewards/catalog';
import { FontSize, Spacing } from '@/constants/theme';
import { useDiceAccentColor } from '@/hooks/use-dice-accent-color';
import { useThemedStyles } from '@/hooks/use-themed-styles';
import { applyDiceKeepMode, CHAT_DICE_SKINS_ENABLED, DICE_CRIT_FAIL_LABEL, DICE_CRIT_SUCCESS_LABEL, diceFaceMark, diceRollCritLabels, formatDiceFormula, payloadToStageNotation, } from '@/utils/chat-dice-roll';
...
function canAnimateIncoming(request, animationSpeed, isFocused) {
...
function normalizeOutcomeGroups(outcome, dice) {
    ...
    for (const group of outcome.groups) {
        ...
        const sides = typeof rawSides === 'number'
            ? rawSides
 ...
 */
export function ChatDiceOverlay({ request, localRoll = null, onLocalRollComplete, onReveal, onAdvance, warm = false, forceActive = false, }) {
    ...
    const activeSkin = CHAT_DICE_SKINS_ENABLED && isDiceSkinId(localRoll?.skin ?? request?.payload.skin)
        ? (localRoll?.skin ?? request?.payload.skin)
        : 'standard';
    const activeAccent = coerceDiceAccent(activeSkin !== 'standard'
    ...
    const startedLocalTokensRef = useRef(new Set());
    useEffect(() => {
        if (!localRoll) {
            return;
        }
        ...
        const clientFallbackOutcome = () => {
            const expected = [...active.dice].sort((a, b) => a.sides - b.sides);
            const groups = expected.map((die) => {
                const values = Array.from({ length: die.qty }, () => Math.floor(Math.random() * die.sides) + 1);
                return {
                    sides: die.sides,
                    values,
                    sum: values.reduce((a, b) => a + b, 0),
        ...
        const notation = notationParts.length === 0
            ? '1d20'
        ...
        void (async () => {
            ...
            try {
                ...
                const rollPromise = stageRef.current?.roll(notation) ?? Promise.resolve(null);
                ...
                if (!raw) {
                    ...
                    done = applyDiceKeepMode(normalizeOutcomeGroups(raw, active.dice), rollMode, active.modifier);
                    // Куб уже на нужных гранях — карточка = тот же outcome.
                    const faceRows = done.groups.flatMap((group) => group.values.map((value) => ({ sides: group.sides, value })));
                    setResult({
    ...
    useEffect(() => {
        ...
        const notationParts = payloadToStageNotation(active.payload).map((part) => `${part.qty}d${part.sides}`);
        const notation = notationParts.length === 0
            ? '1d20'
            : notationParts.length === 1
                ? notationParts[0]
                : notationParts;
        const faces = active.payload.groups.flatMap((g) => g.values.map((value) => ({ sides: g.sides, value })));
        const sum = active.payload.sum;
        const critLabels = diceRollCritLabels(active.payload.groups, active.payload.mode, active.payload.modifier);
        void (async () => {
            let revealed = false;
            try {
                await new Promise((resolve) => {
                ...
                const faceMismatch = !rolled ||
                    expectedValues.length === 0 ||
                    expectedValues.some((value, index) => Number(rolled.values[index]) !== Number(value));
                if (faceMismatch) {
                    stageRef.current?.clear();
                }
    ...
    const visible = Boolean(localRoll) ||
        (Boolean(request) &&
            animationSpeed !== 'off' &&
            isFocused &&
            Boolean(request && !request.payload.redacted && request.payload.sum != null));
    const throwerNickname = localRoll?.senderNickname ?? (visible ? request?.senderNickname : null) ?? null;
    ...
    const overlay = (<View style={[
      ...
      <View style={styles.stage} pointerEvents="none">
        {engineMounted || visible || keepStageHot ? (<DiceStage ref={stageRef} accent={activeAccent} skin={activeSkin} transparent animationSpeed={animationSpeed === 'off' ? 'normal' : animationSpeed} onReady={handleStageReady}/>) : null}
        <DiceCritBurst visible={Boolean(result?.critLabels.includes(DICE_CRIT_SUCCESS_LABEL))} skinId={activeSkin}/>
      </View>
      {visible && throwerNickname && !result ? (<View style={[styles.throwerBanner, { top: Math.max(insets.top, Spacing.md) + Spacing.sm }]} pointerEvents="none">
          <View style={styles.throwerChip}>
            <Text style={styles.throwerLabel}>Бросает</Text>
            <Text style={styles.throwerName} numberOfLines={1}>
              {throwerNickname}
            </Text>
            {throwerFormula ? (<Text style={styles.resultFormula}>{throwerFormula}</Text>) : null}
          </View>
        </View>) : null}
      {result ? (<View style={styles.resultWrap} pointerEvents="none">
          <View style={styles.resultCard}>
            <Text style={styles.resultWho}>{result.senderNickname}</Text>
            <Text style={styles.resultFormula}>{result.formula}</Text>
            {result.critLabels.length > 0 ? (<View style={styles.resultCritRow}>
            ...
            {result.faces.length > 0 ? (<View style={styles.resultFaces}>
                {result.faces.map((face, index) => {
                    const mark = diceFaceMark(face.sides, face.value, result.modifier);
                    return (<View key={`${index}-${face.sides}-${face.value}`} style={[
                            styles.resultFace,
