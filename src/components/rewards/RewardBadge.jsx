...
import { REWARD_BADGES, rewardUiTone, } from '@/data/rewards/catalog';
...
function clampTip(centerX, top, bottom, viewportW) {
    const place = top > 96 ? 'above' : 'below';
...
export function RewardBadgeIcon({ type, size = 16, interactive = true }) {
    ...
    const measure = useCallback(() => {
        ...
        hostRef.current?.measureInWindow((x, y, width, height) => {
            const viewportW = Platform.OS === 'web' && typeof window !== 'undefined' ? window.innerWidth : x + width + 400;
    ...
    const hoverHandlers = Platform.OS === 'web'
        ...
        : {};
    const overlay = pos && phase !== 'closed'
        ? Platform.OS === 'web'
            ? portalToBody(<div className={`adv-tip ${pos.place === 'above' ? 'is-above' : 'is-below'}${phase === 'out' ? ' is-out' : ''}`} style={{
                    top: pos.top,
                    left: pos.left,
                    ['--accent']: spec.accent,
                    ['--glow']: spec.glow,
                    ['--caret-x']: `${pos.caret}px`,
                }}>
    ...
    if (!interactive) {
        return (<View accessibilityElementsHidden importantForAccessibility="no" pointerEvents="none" style={styles.wrap}>
        <View style={iconStyle}>
          <Ionicons name={ICON_MAP[type]} size={size} color={tone.fg}/>
        </View>
      </View>);
    }
    return (<View ref={hostRef} collapsable={false} style={styles.wrap}>
      <Pressable accessibilityRole="button" accessibilityLabel={spec.label} accessibilityHint={spec.tooltip} onPress={() => (phase === 'in' ? hide() : show())} {...hoverHandlers} style={iconStyle}>
        <Ionicons name={ICON_MAP[type]} size={size} color={tone.fg}/>
      </Pressable>
      {overlay}
    </View>);
}
export function RewardBadgeRow({ badges, size = 14, wrap = false, align = 'start', }) {
    const unique = useMemo(() => [...new Set(badges)].sort((a, b) => a.localeCompare(b)), [badges]);
    if (unique.length === 0) {
        return null;
    }
    return (<View style={[
      ...
      {unique.map((badge) => (<RewardBadgeIcon key={badge} type={badge} size={size}/>))}
    </View>);
}
export function NameWithBadges({ name, badges, textStyle, suffix, badgeSize = 13, layout = 'inline', align = 'start', }) {
    const row = (<RewardBadgeRow badges={badges ?? []} size={badgeSize} wrap={layout === 'stack'} align={layout === 'stack' || align === 'center' ? 'center' : 'start'}/>);
    if (layout === 'stack') {
        return (<View style={[styles.nameStack, align === 'center' && styles.nameStackCenter]}>
    ...
    return (<View style={[styles.nameRow, align === 'center' && styles.nameRowCenter]}>
...
const styles = StyleSheet.create({
    ...
    tooltip: {
        ...
        zIndex: 100000,
        ...Platform.select({
            web: { boxShadow: '0 18px 40px rgba(6, 8, 16, 0.42)' },
