...
import { AVATAR_FRAMES, displayedFrameId, isStaticAvatarFrame, } from '@/data/rewards/catalog';
function OrbitBits({ className, count, duration = 8, ring = 'outer', reverse = false, }) {
    return (<>
      {Array.from({ length: count }, (_, i) => (<View key={`${className}-${ring}-${i}`} pointerEvents="none" {...webFxClass(`${className} adv-fx-spoke${ring === 'inner' ? ' is-inner' : ''}${reverse ? ' is-rev' : ''}`)} style={{
                position: 'absolute',
                left: '50%',
                top: '50%',
...
function FrameExtras({ id, size }) {
    ...
    switch (id) {
        ...
        case 'tide_ring':
          ...
          {mini ? null : (<View pointerEvents="none" {...webFxClass('adv-fx-wave alt')} style={StyleSheet.absoluteFill}/>)}
        ...
        case 'ghost_veil':
          ...
          {mini ? null : (<View pointerEvents="none" {...webFxClass('adv-fx-mist alt')} style={StyleSheet.absoluteFill}/>)}
        ...
        case 'frost_ring':
          ...
          {mini ? null : (<OrbitBits className="adv-fx-star cyan" count={8} duration={14} ring="inner" reverse/>)}
        </>);
        case 'hex_circuit':
...
function OakHoop({ size }) {
    const rivets = orbitCount(10, size);
    const mini = size <= 36;
    return (<>
      <View pointerEvents="none" {...webFxClass('adv-fx-grain')} style={StyleSheet.absoluteFill}/>
      {mini ? null : (<View pointerEvents="none" {...webFxClass('adv-fx-hearth')} style={StyleSheet.absoluteFill}/>)}
      {Array.from({ length: rivets }, (_, i) => (<View key={`oak-rivet-${i}`} pointerEvents="none" {...webFxClass('adv-fx-rivet adv-fx-spoke')} style={{
                position: 'absolute',
                left: '50%',
                top: '50%',
                width: '47%',
...
export function avatarFramePad(size) {
    return Math.max(8, Math.round(size * 0.11));
}
export function avatarFrameOuterSize(size) {
    return size + avatarFramePad(size) * 2;
}
export function AvatarFrame({ size, badges, frameId, children }) {
    ...
    return (<View style={{ width: box, height: box, alignItems: 'center', justifyContent: 'center', overflow: 'visible' }}>
      {Platform.OS === 'web' ? (<WebFrame key={resolved} id={resolved} size={size}/>) : (<NativeFrame id={resolved} size={box}/>)}
...
function WebFrame({ id, size }) {
    ...
    return (<View pointerEvents="none" style={[StyleSheet.absoluteFill, { overflow: 'visible' }]} {...webFxClass(`adv-fx adv-fx--${id}${staticFrame ? ' is-static' : ''}${scale}`)}>
      <View {...webFxClass('adv-fx-glow')}/>
      <View {...webFxClass('adv-fx-track')}/>
      {staticFrame ? null : (<View {...webFxClass('adv-fx-spin')}>
