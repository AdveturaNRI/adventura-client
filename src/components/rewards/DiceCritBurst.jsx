...
import { DICE_SKINS } from '@/data/rewards/catalog';
export function DiceCritBurst({ visible, skinId }) {
    if (!visible || skinId === 'standard') {
        return null;
    }
    return <Burst skinId={skinId}/>;
}
function Burst({ skinId }) {
    ...
    return (<View pointerEvents="none" style={styles.host}>
      {Platform.OS === 'web' ? (<View 
        ...
        className="adv-crit-burst">
          <View 
        // @ts-expect-error web className
        className="adv-crit-core" style={{ backgroundColor: spec.accent, boxShadow: `0 0 48px ${spec.secondary}` }}/>
          <View 
        // @ts-expect-error web className
        className="adv-crit-ring" style={{ color: spec.secondary, borderColor: spec.secondary }}/>
