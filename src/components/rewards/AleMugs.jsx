...
function mugPositions(variant, mug) {
    if (variant === 'card') {
...
export function AleMugs({ size = 52, variant = 'frame' }) {
    ...
    return (<View pointerEvents="none" style={[StyleSheet.absoluteFill, styles.layer]} {...web}>
      <View pointerEvents="none" {...(Platform.OS === 'web' ? webFxClass('adv-mug is-left') : null)} style={[styles.mug, mug, pos.left, Platform.OS !== 'web' ? styles.nativeTiltLeft : null]}>
        <Image source={ALE_MUG} style={[styles.image, styles.flip]} contentFit="contain"/>
      </View>
      <View pointerEvents="none" {...(Platform.OS === 'web' ? webFxClass('adv-mug is-right') : null)} style={[styles.mug, mug, pos.right, Platform.OS !== 'web' ? styles.nativeTiltRight : null]}>
