...
export const EMPTY_QUESTIONNAIRE_AVAILABILITY = {
    byAgreement: false,
    slots: [],
};
const DAY_SHORT_TO_INDEX = Object.fromEntries(WEEKDAY_OPTIONS.map((day) => [day.short, day.index]));
...
export function formatAvailabilityTimeInput(value) {
    return normalizeTime(value);
}
/** Turns partial hour input like "22" or "9" into "22:00" / "09:00". */
export function completeAvailabilityTime(value) {
    const digits = value.replace(/\D/g, '').slice(0, 4);
    if (!digits) {
        return '';
    }
...
function formatTimePart(timeFrom, timeTo) {
...
export function getAvailabilityDays(schedule) {
    return schedule.slots.map((slot) => slot.day).sort((left, right) => left - right);
}
export function formatAvailability(schedule, options) {
...
function parseTimeSuffix(segment) {
...
export function areAvailabilitiesEqual(left, right) {
    ...
    return leftSorted.every((slot, index) => {
        ...
        return (slot.day === other.day &&
            slot.timeFrom === other.timeFrom &&
            slot.timeTo === other.timeTo);
    });
}
export function toggleAvailabilityDay(schedule, dayIndex) {
    const hasDay = schedule.slots.some((slot) => slot.day === dayIndex);
    const slots = hasDay
        ? schedule.slots.filter((slot) => slot.day !== dayIndex)
        : [...schedule.slots, { day: dayIndex, timeFrom: '', timeTo: '' }].sort((left, right) => left.day - right.day);
    return {
        ...schedule,
        byAgreement: false,
        slots,
...
export function updateAvailabilityDayTime(schedule, dayIndex, patch) {
    return {
        ...schedule,
        byAgreement: false,
        slots: schedule.slots.map((slot) => slot.day === dayIndex
            ? {
                ...slot,
                ...patch,
            }
...
export function applyAvailabilityTimeToAll(schedule) {
    const source = schedule.slots.find((slot) => slot.timeFrom.trim() !== '' || slot.timeTo.trim() !== '');
    if (!source) {
        return schedule;
    }
...
export function stripAvailabilityTimes(schedule) {
    return {
        ...schedule,
        slots: schedule.slots.map((slot) => ({
            ...slot,
