...
import type { PlayerRoleChoice } from './types';
...
export const QUESTIONNAIRE_STEPS: QuestionnaireStepKey[] = [
  'roles',
  'profile',
  'experience',
  'location',
...
export type RoleOptionSpec = {
  key: PlayerRoleChoice;
  label: string;
  description: string;
...
export const QUESTIONNAIRE_ENTRY = '/questionnaire' as const;
...
export const QUESTIONNAIRE_AFTER_FINISH_HREF = '/wanderers' as const;
export const QUESTIONNAIRE_UNSAVED_TITLE = 'Есть несохранённые изменения';
export const QUESTIONNAIRE_UNSAVED_MESSAGE =
  'Сохранить изменения на этом шаге перед выходом из анкеты?';
...
export const QUESTIONNAIRE_DELETE_MESSAGE =
  'Все данные анкеты будут удалены: фото, статус, описание, опыт, локацию и системы. Никнейм аккаунта сохранится.';
...
export const ROLES_STEP = {
  ...
  hint:
    'Роль можно изменить в любой момент в анкете. Если вы только знакомитесь с настольными ролевыми играми — смело выбирайте «Игрок»: мастером можно стать позже, когда появится желание вести свои истории.',
  options: [
    {
      key: 'player',
      label: 'Игрок',
...
export const PROFILE_STEP = {
  ...
  hint:
    'Никнейм виден всем в приложении. Статус — короткая фраза о вас. Фото анкеты — ваша карточка в ленте Странники, где игроки знакомятся и собирают компанию для игр.',
  profileCardLabel: 'Фото анкеты',
  profileCardDescription:
    'Большое фото для ленты Странники — покажите себя так, как вам нравится. Необязательное поле.',
  ...
  visibilityPublicHint:
    'Публичная анкета попадает в ленту Странники только после заполнения обязательных полей: роль, статус, возраст, опыт, расписание, часовой пояс, локация и системы.',
  visibilityPrivateHint:
    'Приватная анкета не показывается в ленте. Её увидят только те, кому вы откроете доступ после заявки на игру.',
};
...
export const EXPERIENCE_STEP = {
  ...
  hint:
    'Эти данные помогают мастерам и игрокам быстрее понять, подходите ли вы друг другу.',
  ...
  availabilityPanelSubtitle:
    'Отметьте дни и при желании укажите время для каждого — или выберите «по договорённости»',
...
export const LOCATION_STEP = {
  ...
  subtitle: 'Укажите город или отметьте, что играете онлайн',
  hint:
    'Локация помогает находить офлайн-игры рядом. Если играете только онлайн — можно не указывать город.',
  cityLabel: 'Город',
  cityPlaceholder: 'Начните вводить название',
  cityEmptyHint: 'Пока не выбран',
...
export const FINAL_STEP = {
  ...
  hint:
    'Проверьте карточку и статус видимости. Анкета попадает в ленту только если заполнены обязательные поля и включён режим «Публичная».',
