...
type RingMode = 'incoming' | 'ringback';
...
let activeMode: RingMode | null = null;
...
function playIncomingBurst(context: AudioContext) {
 ...
 */
function playRingbackBurst(context: AudioContext) {
  const now = context.currentTime;
  const duration = 1.05;
  const master = context.createGain();
  master.gain.setValueAtTime(0.0001, now);
...
function startLoop(mode: RingMode, intervalMs: number, play: (ctx: AudioContext) => void) {
  if (!canRing()) {
    return;
  }
  stopCallRingtone();
 ...
 */
export function playMicToggleSound(muted: boolean) {
  if (!canRing()) {
    return;
  }
  const context = getContext();
