...
export type WebMediaGainHandle = {
  ...
  /** Call after src change on the same element (or after replace creates a new one). */
...
/**
 * Safari/iOS ignore HTMLMediaElement.volume. Route through GainNode.
 * Only safe on same-origin (blob:) or CORS-enabled media — otherwise silence.
 */
export function createWebMediaGain(media: HTMLMediaElement): WebMediaGainHandle | null {
  ...
  const bind = (nextMedia: HTMLMediaElement) => {
    ...
    try {
      // Element must play through this graph once connected.
