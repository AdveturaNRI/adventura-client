...
const FALLBACK_PRESETS: NotificationSoundPreset[] = [
  {
    id: 'nsound_magic',
    slug: 'magic',
...
let cachedSettings: NotificationSoundSettings = { ...DEFAULT_SETTINGS };
let cachedPresets: NotificationSoundPreset[] = [...FALLBACK_PRESETS];
let hydrated = false;
...
function parsePresets(raw: string | null): NotificationSoundPreset[] {
  if (!raw) {
    return [...FALLBACK_PRESETS];
  }
  try {
    const parsed = JSON.parse(raw) as NotificationSoundPreset[];
...
async function writeSettingsLocal(settings: NotificationSoundSettings) {
  const payload = JSON.stringify(settings);
  await AsyncStorage.setItem(STORAGE_KEY, payload);
  writeWeb(STORAGE_KEY, payload);
}

async function writePresetsLocal(presets: NotificationSoundPreset[]) {
...
export function getNotificationSoundSettingsSync(): NotificationSoundSettings {
  if (!hydrated) {
...
export function getNotificationSoundPresetsSync(): NotificationSoundPreset[] {
  getNotificationSoundSettingsSync();
  return cachedPresets;
}

export function getEffectiveNotificationSoundUrl(): string | null {
  ...
  const preset =
    cachedPresets.find((item) => item.id === settings.presetId) ??
    cachedPresets.find((item) => item.isDefault) ??
    cachedPresets[0];
  return preset?.url ?? null;
}

export async function fetchNotificationSoundPresets(): Promise<NotificationSoundPreset[]> {
  try {
    const remote = await apiRequest<NotificationSoundPreset[]>('/notification-sounds', {
...
export async function loadNotificationSoundSettings(): Promise<NotificationSoundSettings> {
...
export async function saveNotificationSoundSettings(
  next: NotificationSoundSettings,
  options?: { syncRemote?: boolean },
): Promise<NotificationSoundSettings> {
  const settings: NotificationSoundSettings = {
...
export async function hydrateNotificationSoundSettingsFromProfile(): Promise<NotificationSoundSettings> {
  ...
  try {
    ...
    const useCustom = Boolean(
      profile.useCustomNotificationSound && profile.customNotificationSoundUrl,
    );
    const settings: NotificationSoundSettings = {
      enabled:
        typeof profile.notificationSoundsEnabled === 'boolean'
