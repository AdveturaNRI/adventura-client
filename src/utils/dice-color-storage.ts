...
export type DiceAccentOption = (typeof DICE_ACCENT_PALETTE)[number];
...
export function coerceDiceAccent(raw: string | null | undefined): string {
  const normalized = normalizeHex(raw);
  if (!normalized) return DEFAULT_DICE_ACCENT;
  const match = DICE_ACCENT_PALETTE.find(
    (item) => item.hex.toLowerCase() === normalized.toLowerCase(),
  );
  return match?.hex ?? normalized;
}

export function getDiceAccentColorSync(): string {
...
export async function loadDiceAccentColor(): Promise<string> {
  const value = await AsyncStorage.getItem(KEY);
  return coerceDiceAccent(value);
}

export async function saveDiceAccentColor(hex: string): Promise<void> {
