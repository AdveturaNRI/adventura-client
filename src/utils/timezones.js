...
function offsetMinutes(timeZone) {
    try {
        ...
        const raw = formatter.formatToParts(new Date()).find((part) => part.type === 'timeZoneName')
            ?.value ?? '';
...
export function formatTimezoneSubtitle(timeZone) {
    const id = timeZone?.trim() || DEFAULT_TIMEZONE;
    const option = getTimezoneOptions().find((item) => item.id === id);
    if (option) {
...
export function getZonedDateTimeParts(isoOrDate, timeZone = DEFAULT_TIMEZONE) {
...
export function wallTimeToUtcIso(year, month, day, hour, minute, timeZone = DEFAULT_TIMEZONE) {
...
export function formatDateTimeInTimezone(iso, timeZone, options) {
    if (!iso) {
        return null;
    }
    try {
