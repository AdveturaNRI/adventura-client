...
import { useChatLiveVoice, } from '@/hooks/use-chat-live-voice';
import { acceptChatVoiceCall, declineChatVoiceCall, endChatVoiceCall, inviteChatVoiceCall, joinChatVoiceCall, } from '@/services/chats/chatsApi';
...
function mapInviteRinging(peers) {
    if (!peers?.length) {
        return [];
    }
...
function dropRingingPeer(peers, userId) {
...
export function VoiceCallProvider({ children }) {
    ...
    const ringingStartedAtRef = useRef(null);
    const hadRemoteRef = useRef(false);
    const liveConversationId = session && (session.phase === 'outgoing' || session.phase === 'active')
        ? session.conversationId
        : null;
    const { status: liveStatus, error: liveError, muted, deafened, cameraOn, participants, urgentUntilById, join, leave, toggleMute, toggleDeafen, toggleCamera, sendUrgentRequest, } = useChatLiveVoice(liveConversationId);
    const clearSession = useCallback(async (opts) => {
        if (opts?.playHangup) {
            stopCallRingtone();
            playHangupSound();
        }
    ...
    const startCall = useCallback(async (conversationId, peerName, peerAvatarUrl, opts) => {
        ...
        try {
            ...
            const ringingPeers = mapInviteRinging(ringing).length > 0
    ...
    const joinOngoingCall = useCallback(async (conversationId, callId, peerName, peerAvatarUrl, opts) => {
        if (sessionRef.current) {
            toast.info('Сначала завершите текущий звонок');
            return;
        }
    ...
    const acceptIncoming = useCallback(async () => {
        ...
        stopCallRingtone();
        const next = { ...current, phase: 'active', ringingPeers: [] };
        sessionRef.current = next;
        setSession(next);
        setMinimized(false);
        hadRemoteRef.current = false;
        ...
        return subscribeCallEvents((event) => {
            ...
            if (event.type === 'invite') {
                ...
                const next = {
                    ...
                    role: 'callee',
                    peerName: (isGroup ? invite.conversationTitle?.trim() : null) ||
                        invite.fromNickname ||
                        'Собеседник',
            ...
            if (event.type === 'accepted') {
                ...
                if (ringingPeers.length !== current.ringingPeers.length) {
                    const next = { ...current, ringingPeers };
                    sessionRef.current = next;
                    setSession(next);
                }
                return;
    ...
    useEffect(() => {
        ...
        const inCall = session?.phase === 'outgoing' ||
    ...
    const phase = session?.phase ?? (liveStatus === 'error' ? 'error' : 'idle');
    const waitingPeers = useMemo(() => (session?.ringingPeers ?? []).map((peer) => ({
        id: peer.userId,
        name: peer.nickname,
    ...
    const overlayVisible = session?.phase === 'outgoing' ||
        session?.phase === 'active' ||
    ...
    return (<VoiceCallContext.Provider value={value}>
      <View style={providerStyles.root}>
        <VoiceCallOverlay visible={Boolean(overlayVisible)} minimized={minimized} title={overlayTitle} isGroup={Boolean(session?.isGroup)} status={liveStatus} ringing={session?.phase === 'outgoing'} error={liveError} muted={muted} deafened={deafened} cameraOn={cameraOn} participants={participants} waitingPeers={waitingPeers} urgentUntilById={urgentUntilById} onToggleMute={() => void toggleMute()} onToggleDeafen={() => void toggleDeafen()} onToggleCamera={() => void handleToggleCamera()} onHangup={() => void hangup()} onRetry={() => void retryLive()} onMinimize={minimize} onExpand={expand} onUrgentRequest={() => void sendUrgentRequest()} conversationId={session?.conversationId ?? null} diceSenderNickname={user?.nickname ?? 'Вы'}/>
        {children}
        <IncomingCallModal visible={session?.phase === 'incoming'} callerName={session?.peerName ?? ''} callerAvatarUrl={session?.peerAvatarUrl ?? null} subtitle={session?.isGroup && session.callerName
            ? `${session.callerName} звонит в группу`
            : session?.isGroup
                ? 'Групповой звонок'
                : undefined} onAccept={() => void acceptIncoming()} onDecline={() => void declineIncoming()}/>
      </View>
