...
export const QUESTIONNAIRE_AURAS = {
...
export const SHOWCASE_AVATAR_FRAME_IDS = AVATAR_FRAME_IDS.filter((id) => id !== 'none');
export function isStaticAvatarFrame(_id) {
    return false;
}
export function isUniqueAvatarFrame(id) {
    return Boolean(AVATAR_FRAMES[id]?.unique);
}
export function isPurchasableAvatarFrame(id) {
    return Boolean(AVATAR_FRAMES[id]?.purchasable);
}
export function isUniqueAura(id) {
    return Boolean(QUESTIONNAIRE_AURAS[id]?.unique);
}
export function isPurchasableAura(id) {
    return Boolean(QUESTIONNAIRE_AURAS[id]?.purchasable);
}
export function wildcardAvatarFrameIds() {
    return SHOWCASE_AVATAR_FRAME_IDS.filter((id) => isPurchasableAvatarFrame(id));
}
export function wildcardAuraIds() {
...
export function isRewardBadgeType(value) {
    return typeof value === 'string' && REWARD_BADGE_TYPES.includes(value);
}
export function isDiceSkinId(value) {
    return typeof value === 'string' && DICE_SKIN_IDS.includes(value);
}
...
export function frameIdForBadges(badges) {
    const badge = primaryBadge(badges);
    return badge ? REWARD_BADGES[badge].frameId : 'none';
}
export function auraIdForBadges(badges) {
    const badge = primaryBadge(badges);
    if (!badge) {
        return 'none';
    }
...
export function exclusiveSkinIds(badges) {
    return [
        ...new Set(badges
...
export function isAvatarFrameId(value) {
    return typeof value === 'string' && AVATAR_FRAME_IDS.includes(value);
}
export function isQuestionnaireAuraId(value) {
    return typeof value === 'string' && QUESTIONNAIRE_AURA_IDS.includes(value);
}
export function ownedFrameIdsForBadges(badges) {
    const ids = [];
...
export function sanitizeOwnedFrameIds(raw) {
    if (!Array.isArray(raw)) {
        return [];
    }
    const ids = [];
...
export function ownedFrameIdsFromPerks(perks, badges) {
...
export function ownedAuraIdsForBadges(badges) {
    const ids = [];
    for (const badge of badges) {
...
export const SHOWCASE_AURA_IDS = QUESTIONNAIRE_AURA_IDS.filter((id) => id !== 'none');
export const SHOWCASE_DICE_SKIN_IDS = DICE_SKIN_IDS.filter((id) => id !== 'standard');
export function sanitizeOwnedAuraIds(raw) {
    if (!Array.isArray(raw)) {
        return [];
    }
    const ids = [];
...
export function ownedAuraIdsFromPerks(perks, badges) {
...
export function diceSkinIdsFromPerks(perks) {
    ...
    const ids = new Set(['standard', ...fromPerks, ...fromBadges, ...fromWildcard]);
