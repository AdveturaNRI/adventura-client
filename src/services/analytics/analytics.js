...
const localViewSentAt = new Map();
const localTransitionSentAt = new Map();
...
function shouldSendLocal(map, entity, id) {
...
function mirrorToYandexMetrika(name, props) {
    if (Platform.OS !== 'web') {
        return;
    }
...
export function trackUserSessionStarted(method = 'app_open') {
    const now = Date.now();
...
export function trackEntityView(entity, id) {
...
export function trackEntityTransition(entity, id) {
    const trimmed = id.trim();
    ...
    void apiRequest('/analytics/transitions', {
        method: 'POST',
        body: {
            entity,
            id: trimmed,
