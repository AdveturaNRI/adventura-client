...
function normalizeCounterId(raw) {
...
export function getYandexMetrikaId() {
    ...
    return (resolvedId ??
        normalizeCounterId(typeof window !== 'undefined' ? window.__ADVENTURA_YM_ID__ : null) ??
...
function loadTagScript(counterId) {
    if (typeof document === 'undefined') {
        return Promise.resolve();
    }
    ensureYmStub();
    const src = `https://mc.yandex.ru/metrika/tag.js?id=${counterId}`;
    const existing = document.querySelector('script[src*="mc.yandex.ru/metrika/tag.js"]');
    if (existing) {
    ...
    try {
        ...
        return {
            ...
            config: {
                ...
                accurateTrackBounce: payload.yandexMetrika?.accurateTrackBounce ?? true,
            },
        };
    }
    catch {
 ...
 */
export async function bootstrapYandexMetrika() {
...
export function hitYandexMetrika(url, options) {
...
export function reachYandexMetrikaGoal(goal, params) {
    const id = getYandexMetrikaId();
    if (!id || !goal.trim()) {
        return;
    }
...
export function setYandexMetrikaUserId(userId) {
    const id = getYandexMetrikaId();
    if (!id) {
        return;
    }
