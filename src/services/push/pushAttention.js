...
import { fetchPushStatusForThisDevice, getCurrentPushEndpoint, getNotificationPermission, isIosSafariNeedPwaHint, isWebPushSupported, } from '@/services/push/webPush';
...
export async function isPushAttentionDismissed() {
    try {
...
export async function dismissPushAttention() {
    await AsyncStorage.setItem(DISMISSED_KEY, '1');
}
export async function clearPushAttentionDismissed() {
    await AsyncStorage.removeItem(DISMISSED_KEY);
}
export async function isPushUserDisabled() {
    try {
...
export async function markPushUserDisabled() {
    await AsyncStorage.setItem(USER_DISABLED_KEY, '1');
}
export async function clearPushUserDisabled() {
    await AsyncStorage.removeItem(USER_DISABLED_KEY);
}
export async function isSoftPromptDismissed() {
    try {
...
export async function markSoftPromptDismissed() {
    await AsyncStorage.setItem(SOFT_DISMISS_UNTIL_KEY, String(Date.now() + SOFT_DISMISS_MS));
}
export async function clearSoftPromptDismissed() {
...
export async function markOfferPushAfterAuth() {
    if (Platform.OS !== 'web') {
        return;
    }
...
export async function markOfferPushAfterRegister() {
    return markOfferPushAfterAuth();
}
export async function peekOfferPushAfterAuth() {
    try {
...
export async function consumeOfferPushAfterAuth() {
    try {
...
export async function consumeOfferPushAfterRegister() {
    return consumeOfferPushAfterAuth();
}
export async function hasOfferedFcmV1() {
    try {
...
export async function markFcmV1Offered() {
 ...
 */
export async function deviceNeedsPushOptIn() {
...
export async function resolvePushOptInVariant() {
    const permission = getNotificationPermission();
 ...
 */
export async function shouldForceLegacyFcmOffer() {
...
export async function shouldShowPushAttention() {
    if (Platform.OS !== 'web' || !isWebPushSupported()) {
        return false;
    }
